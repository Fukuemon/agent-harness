...
agent-harness は、3 つのパッケージと、それらが共有する schema と例を配るリポジトリである。  
パッケージは、文書の体系、開発プロセス、ガードレールの 3 つで、利用者はマニフェストに書いて 1 つずつ導入する。  
利用者のリポジトリでは、パッケージのスキルとテンプレートとチェックが、Claude Code と Codex CLI の作業を支える。Cursor は可能な範囲で対応する。

- [PRD](../PRD.md) — 課題、目標、受け入れの条件

この文書は全体像だけを持つ。機能ごとの設計は、[構成要素](#構成要素)の節から機能ごとの Design Doc へリンクする。
...
構成要素は、利用者のリポジトリの共通の基盤、3 つのパッケージ、パッケージが共有するものである。
...
- どのパッケージを外しても `context/` は残る。文書の体系は、その上に文書の形とチェックを足すパッケージである。

### パッケージ

パッケージは互いに依存させない。1 つを外しても残りが動く。

- **文書の体系 `docs`:** Design Doc、context、ADR、spec の構造と、どの情報をどの文書に書くかのルール、文書のテンプレート、文書のチェックを持つ。利用者の知識の中身は持たない。内容の正しさは判定しない。
...
- **schema:** プロジェクトごとの値のファイルと、要求ごとの宣言のファイルの形を定める。`schemas/` に置く。値そのものは持たない。
...
2. 文書の体系の導入のスキルを呼ぶ。スキルが保護ブランチの名前や置く context の話題を尋ね、付属のスクリプトが `context/project.yml`、目次、AGENTS.md、CONTRIBUTING.md、設定のファイルを写す。存在するファイルは上書きしない。
3. 写されたファイルをコミットする。以後は利用者のものとして編集する。
4. Git のフックを使うなら、`lefthook.yml` に remotes を書き、`lefthook install` を実行する。

文書の体系を入れない利用者は、`examples/project.yml` を `context/` へ手で写す。
...
├── packages/
│   └── <パッケージの名前>/    docs、process、guardrails
│       ├── .claude-plugin/
│       │   └── plugin.json
