...
context は、技術スタックの規約、コードベースの構造の取り決め、運用の取り決めを持つ。現在の取り決めだけを書き、モデルがコードを読んでも分からない事実に限る。
...
目次は `context/index.md` で、`context/` と、文書の体系を入れていれば `design/` の frontmatter から生成する。手で編集しない。手書きの更新日も持たない。日付は更新し忘れを検出できないが、commit と履歴の差分は検出できる。
...
- スクリプトが、`examples/project.yml` と `assets/` のテンプレートを写す。写したファイルの一覧を表示する。
- 写すのは、`context/project.yml`、選んだ話題の context、`context/index.md`、AGENTS.md、CONTRIBUTING.md である。ほかのパッケージが配置されていれば、そのテンプレート（文書の骨組み、設定のファイル、issue と PR のテンプレート）も写す。配置されていないパッケージの分は飛ばす。
