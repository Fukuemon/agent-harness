...
- 持つのは、プロセスの定義、選択の項目と選択肢、YAML の schema、チェック、プロセスの中で使う手段である。手段は、コミットとブランチ名の規約のスキル、issue と pull request のテンプレート、コードのコメントの規約のスキルと編集後のフックである。
- 分類: 開発プロセスの標準。
...
| `integration` | 取り込み | 変更を基準のブランチへ入れる | 取り込まれた pull request  | pull request が取り込まれ、issue が閉じられている |
...
- issue と pull request のテンプレートは、利用者のリポジトリへ写す。
...
flowchart TD
    ...
    close --> propose["CI が、spec のディレクトリを削除する pull request を作る"]
