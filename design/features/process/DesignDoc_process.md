...
- 持つのは、プロセスの定義、選択の項目と選択肢、YAML の schema、進み具合のチェックである。コミット、ブランチ、issue と pull request、コードのコメントの規約は、core の運用の取り決めが持つ。
...
flowchart TB
    subgraph AH["agent-harness: 定義を持つ"]
        ah["プロセスの定義 / 選択の項目と選択肢<br>YAML の schema / 進み具合のチェック"]
    end
