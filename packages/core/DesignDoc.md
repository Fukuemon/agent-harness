...
- context と Design Doc の境界は、次のとおりである。context は、機能をまたいで成り立つ取り決めと契約と、それがどこに書いてあるかの所在を持つ。Design Doc は、構成と処理の流れを持ち、対象のコードの隣に置く。基盤なら `infra/DesignDoc.md` のように置き、context の基盤と運用はデプロイの条件や秘密情報の方針を持って、構成はその Design Doc を指す。spec は要求ごとに置く文書で、issue を閉じるときに消えるので、残る事実の置き場にしない。
- 所在は、本文のリンクで示す。文書の体系を入れていれば、frontmatter の `governs` にそのディレクトリを書き、実装とのずれの検出の対象にする。
- モノレポでアプリごとに違う取り決めは、`context/<種類>/<項目>.md` に分けて持つ。context は横断の取り決めなので、コードの隣には置かない。
- テンプレートは、種類ごとに用意する。導入のときに 6 種類すべてを `status: draft` の骨組みとして置く。どの種類も要るが、決まる時期が違うためである。中身は、決まる時期に、context を書くスキルで書く。
- 種類は `context/<種類>.md` の 1 ファイルにする。1 ファイルに収まらない種類は、`context/<種類>.md` に一覧を置いて、`context/<種類>/<項目>.md` に分ける。

| 種類 | 決まる時期 | 書くこと |
| --- | --- | --- |
| 技術スタック | 要求と設計が固まったとき | 採用しているツールの役割と、版が書いてあるファイルの場所。版はマニフェストが持ち、選んだ理由は ADR が持つので書かない |
| コードベースの構造 | 実装に入るとき | モジュールの境界と、依存してよい向き、状態の置き場。コードから読み取れる構造は書かない |
| コードの規約 | 実装に入るとき | 命名、コメント、共有の設定、自動チェックの除外。lint で検出できる規則は書かない |
| テスト | 検証の設計のとき | テストの種類ごとの責務と、実行に要る環境の条件 |
| 基盤と運用 | 取り込みとリリースのとき | 環境の種類、デプロイと切り戻しの条件、監視、秘密情報の置き場の方針 |
| 業務の知識 | 要求のとき | 用語と、概念ごとの文書の一覧。状態と遷移、不変条件と禁止事項は、概念ごとに `context/domain/<概念>.md` に置く。それらを実現する構成と流れは、機能ごとの Design Doc が持つ |

- テンプレートの節は、モデルがコードを読んでも分からない事柄に限る。境界の理由、依存させない向き、動かす環境などである。見出しは、Design Doc の骨組みと同じく英語の名詞で書く。
...
- 生成するのは `hooks/build-index.mjs` である。既定は生成し直した目次と `context/index.md` を比べ、差分があれば終了コード 1、必須のキーが欠けていれば終了コード 2 で報告する。`--write` は目次を書き換える。
- 文書を編集した後のフックは `--write` で動き、目次を書き換える。判断が要らない生成なので、確かめるよう促す文は追加しない。lefthook と CI は `--write` を付けずに動かし、ずれを失敗として報告する。
- 導入のスキルの `setup.mjs` も、写す文書から同じ形の目次を作る。生成の関数は 2 つのスクリプトに同じものを持つ。パッケージマネージャーはフックのスクリプトを 1 ファイルだけ写すので、実行時に共有できないためである。2 つの出力が一致することは、テストで確かめる。
...
context を種類ごとに決めて書き、更新するときと、CONTRIBUTING.md と AGENTS.md を直すときに読むスキルを持つ。導入のスキルが置いた骨組みを、決まる時期に埋めるのがこのスキルの役割である。テンプレートの見出しの下の 1 行の案内は「何を書くか」だけを持ち、次はこのスキルが持つ。

- 種類ごとの決まる時期の目安と、決めるときに確かめること。時期は上の表のとおりで、開発プロセスの段階に対応する。
- 種類ごとの、書かないこと。コードから読み取れる事実、選んだ理由、機能ごとの仕様、値そのもの。
- 1 ファイルに収まらない種類を、`context/<種類>/<項目>.md` に分ける規則。業務の知識の概念ごとの文書は、States and Transitions、Invariants、Prohibitions の 3 つの節で作る。
- `status` の意味。`draft` は骨組みだけで、読む側は内容を当てにしない。書き終えたら `stable` にする。
...
- 動くフック: 目次の生成の編集後のフック。コードのコメントの編集後のフック。
