...
- **プロセスと宣言の定義:** 開発のプロセスの定義、要求ごとに決める点と選択肢、宣言の schema を持つ。作業を次へ進める制御は持たない。
- **文書のチェック:** 目次の生成、実装とずれた可能性の一覧、spec と宣言の消し忘れ、宣言の形式、リンク切れを確認する。内容の正しさは判定しない。
...
- [開発のプロセスは種類と決める点だけを定め、選んだ結果は YAML で宣言する](adr/0010-process-and-tailoring.md)
...
sequenceDiagram
    ...
    box 利用者のリポジトリ
        participant D as 宣言<br>プロジェクトの既定 / 要求ごとの YAML
        participant S as 成果物<br>spec / コード / Design Doc / ADR
        participant C as チェックと自動の処理<br>agent-harness から導入したフックと CI
    end

    Note over A: ワークフローのハーネスを導入した場合は、<br>ハーネスが宣言を読んで、この流れを進める
    U->>A: 要求を渡す
    A->>+D: プロジェクトの既定を読む
    D-->>-A: 既定の値を返す
    A->>D: 選んだ 4 つの点を、要求ごとの YAML に書く
    loop 選んだプロセスごと。順序は定めない
        A->>S: プロセスを行い、成果物を書く
        A->>D: 進み具合を書く
        A->>+C: コミットして、チェックを実行する
        C->>+D: 宣言を読む
        D-->>-C: 宣言を返す
        C-->>-A: schema に合わない点を返す
        A->>+U: 選んだ範囲でレビューを依頼する
        U-->>-A: 承認する。または差し戻す
        A->>A: 選んだ時点でタスクに分解し、issue を起票する
        A->>S: 選んだ時点で Design Doc と ADR へ反映する
    end
    U->>+C: issue を閉じる
    C-->>-U: spec と YAML を削除する変更を提案する
    U->>D: 提案を取り込み、要求ごとの YAML を削除する
    U->>S: 同じ変更で、spec とレビュー用の HTML を削除する
```
